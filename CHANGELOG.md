...
- **`mainActivityContains` never matched SIC codes**: the site keeps the code (`SIC`) and the description (`SICD`) in different columns. A numeric value (2-5 digits, prefixes included) is now sent to the code column, text to the description column.
- **A transient HTTP 500 no longer poisons the delta memory.** The site's answer for an unknown record id is a plain 500, identical to a passing server error. A new record whose page is missing is now held back (not stored, not charged, not remembered) until it has been missing in three runs on different days, then delivered as a listing-only record; a known open record keeps being re-checked until it has been missing in two runs; and a batch in which 30% of the records (or five in a row) go missing at once fails the run as a site outage instead of stubbing or closing anything. `fetchOptional` also retries a 500 twice before treating it as missing.
