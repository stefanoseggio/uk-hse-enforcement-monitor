...
// Regulation - the Act/Regulation text is already inline here, no separate
// per-breach detail page exists for notices (unlike convictions). Verified
// live 2026-09-06.
export function parseNoticeBreachList($: CheerioAPI): NoticeBreach[] {
    ...
    $('table tr').each((_i, row) => {
        ...
        const actOrRegulation = $(cells[3]).text().replace(/\s+/g, ' ').trim();
        breaches.push({ breachId, actOrRegulation: actOrRegulation || null });
    });
    return breaches;
}
