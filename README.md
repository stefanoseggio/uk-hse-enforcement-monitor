...
| `maxItemsPerDataset`                 | integer  | `100`                        | Cap on delivered records per register per run (and on cost). Max 100,000. In delta mode the overflow is delivered by the following runs (a walk watermark makes the next run walk down to where this one stopped); on the first delta run the cap defines the baseline                                                                                           |
...
5. **Walk watermark.** HSE enters notices in batches, so a run can meet more new notices than `maxItemsPerDataset`. Stopping at the cap would leave the older part of the batch _under_ the records just delivered, where "two known pages" would hide it forever. The memory therefore also keeps, per register, the notice number where a capped walk stopped (or the lowest candidate a run could not store); the next run ignores known pages until it has walked down to that number, delivers the backlog, and clears the watermark. Only the very first delta run is different: its cap defines the baseline, and what lies below is deliberately never delivered.
