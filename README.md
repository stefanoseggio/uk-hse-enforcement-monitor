...
| Provenance                         | `detailFetched`, `detailError` (`NOT_FOUND` when a record was withdrawn between listing and detail), `breachDetailFetched`, `partyDetailFetched`, `partyDetailError`, `contentHash` (the delta engine's change key), `firstSeenAt`                                                                                                                                                                                                                                                                    |
...
| `mainActivityContains`               | string   | -                            | SIC code or activity text (`43910`, `ROOFING`)                                                                                                                                                                              |
...
| `noticeTypes`                        | string[] | `[]`                         | Notices: `01`-`03` Improvement (incl. Crown), `04`/`05` Deferred / Immediate Prohibition (Crown), `06` Deferred Prohibition, `07` Prohibition (FEPA), `08` Immediate Prohibition, `09` Prohibition (COMAH). Several = union |
| `act`                                | string   | -                            | Notices: Act code, e.g. `502` Health and Safety at Work Act 1974, `517` Corporate Manslaughter (43 codes in the form)                                                                                                       |
| `eventTypes`                         | string[] | all three                    | Which of `SANCTION`, `NEW_LISTING`, `UPDATED` to deliver                                                                                                                                                                    |
...
| `maxItemsPerDataset`                 | integer  | `100`                        | Cap on delivered records per register per run (and on cost). Max 100,000                                                                                                                                                    |
...
| `maxConcurrency`                     | integer  | `5`                          | Parallel record requests (1-10)                                                                                                                                                                                             |
...
**Compliance-date lead generation: open Improvement Notices, updates only**

```json
{
    "datasets": ["notices"],
    "noticeTypes": ["01", "02", "03"],
    "onlyNew": true,
...
4. Memory is written **only for records that were actually stored** - and records are delivered oldest-first - so a spending limit, a timeout or a platform migration half-way never loses a record: the next run simply picks it up. The memory holds 50,000 entries per register (more than the whole notices register).
5. Different filter sets get different memories automatically; set `deltaStateName` to share one deliberately, `resetState: true` to start over. A quiet day costs a few page fetches, the re-checks and the start fee - no records, no charge.
...
- The site is a 2013-era classic-ASP application; if HSE migrates it, extraction breaks. The Actor validates every page and fails loudly (never "0 results, success"), so your schedule alerts you.
